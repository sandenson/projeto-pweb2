...
CREATE TABLE `projeto_pweb_sbll`.`pets` ( `id` INT NOT NULL AUTO_INCREMENT , `name` VARCHAR(100) NOT NULL , `description` TEXT NOT NULL , `type` VARCHAR(100) NOT NULL , `sex` VARCHAR(100) NOT NULL , `isAdopted` BOOLEAN NOT NULL DEFAULT FALSE , `adoptedBy` VARCHAR(100) , `registeredBy` VARCHAR(100) , `adoptionLocation` VARCHAR(256) , PRIMARY KEY (`id`) , FOREIGN KEY (`adoptedBy`) REFERENCES `users`(`username`) , FOREIGN KEY (`registeredBy`) REFERENCES `users`(`username`));
CREATE TABLE `projeto_pweb_sbll`.`pets_imgs` (`petId` INT NOT NULL , `name` VARCHAR(255) NOT NULL , PRIMARY KEY (`petId`) , FOREIGN KEY (`petId`) REFERENCES `pets`(`id`));
