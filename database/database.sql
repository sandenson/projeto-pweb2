...
CREATE TABLE `projeto_pweb_sbll`.`adopt`(`user_fk` VARCHAR(100) NOT NULL , FOREIGN KEY(`user_fk`) REFERENCES `users`(`username`) , `pet_fk` INT NOT NULL , FOREIGN KEY (`pet_fk`) REFERENCES `pets`(`id`));
CREATE TABLE `projeto_pweb_sbll`.`put_to_adoption`(`user_fk` VARCHAR(100) NOT NULL, FOREIGN KEY(`user_fk`) REFERENCES `users`(`username`), `pet_fk` INT NOT NULL, FOREIGN KEY (`pet_fk`) REFERENCES `pets` (`id`));